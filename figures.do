...
* change "cd" (current directory) for your own computer below
cd ""
...
foreach var of local varlist {
	replace `var' = `var' / `units' if treatment != "add person"
}
...
******* display cumulative effects by beginning of 2300


foreach year_treat in "2025" "2025-2030" "2025-2050" "2025-2100" {
	display "year_treat = `year_treat'"
	sum lives_saved_cum births_dif_cum years_lived_saved_cum years_lived_born_cum if year_treat == "`year_treat'" & year == `year_end' - 5 & tfr_scenario == `tfr' & location == "`c'" & life_exp_max == 100
}
...
twoway (line population_born year if tfr_scenario==`tfr' & location=="`c'" & treatment == "add person" & year <= `year_end', lw(`line_thick') lc(`c_born') lp(`p_born')) ///
(line population_saved year if tfr_scenario==`tfr' & location=="`c'" & treatment == "add person" & year <= `year_end' & population_saved > 0, lw(`line_thick') lc(`c_saved') lp(`p_saved')) ///
, ///
 ylab(, nogrid angle(horizontal)) ///
graphr(c(white) lc(white)) ///
 xtitle("Year", size(`axis_size')) ///
...
foreach tfr_ of local tfr_scenarios {
	local gr `gr' line fertility year if location == "`c'" & tfr_scenario == `tfr_' & treatment == "add person" ||
	}
graph twoway `gr', ///
graphr(c(white) lc(white)) ///
...
foreach tfr_ of local tfr_scenarios {
	local graph `graph' line population year if location == "`c'" & tfr_scenario == `tfr_' & treatment == "add person" ||
	}
graph twoway `graph', ///
graphr(c(white) lc(white)) ///
...
if location == "`c'" & treatment == "add person" & year == `year_end' ///
, ///
 ...
 * add person treatment

local x_title = "Country-Specific Total Fertility Rate in 2023"
local y_title = "Descendants Born by 2300 per Life Saved"

twoway (scatter births_dif_cum tfr_2023, mcolor(green)) ///
 if treatment == "add person" & year == `year_end' &  tfr_scenario == `tfr' & location != "`c'",  ///
 legend(off) ///
...
keep if treatment == "add person" & location == "`c'" & tfr_scenario == `tfr' & year == `year_end'
preserve
keep if age_treat == 0
